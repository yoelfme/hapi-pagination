...
module.exports = class Ext {
    ...
    onPostHandler(request, h) {
        ...
        // This is the response that we send from our handler
        ...
        // Now we don't need to pass a totalCount greater than 0, to build the next page URL, because sometimes we don't
        // know the totalCount, so now we can pass the `hasNext` property
        const hasNext = source[this.config.reply.parameters.hasNext.name] || (!Utils.isNil(totalCount) && totalCount !== 0 && currentPage < pageCount);

        // The same thing here as with the `hasNext` variable
        const hasPrevious = (source[this.config.reply.parameters.hasPrevious.name] && currentPage > 1) || (!Utils.isNil(totalCount) && totalCount !== 0 && currentPage > 1);

        // For some endpoints we are going to have a default and unique limit, so we don't want that the user change the limit
        const hasLimit = Utils.isUndefined(source[this.config.reply.parameters.hasLimit.name]) ? true : source[this.config.reply.parameters.hasLimit.name]
        ...
        const getUri = (page) => {
            ...
            if (!hasLimit) {
                const { limit: _removed, _qs } = qs

                return baseUri + Qs.stringify(_qs);
            }
